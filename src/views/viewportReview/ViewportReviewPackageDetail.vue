    ...
    <div>
      ...
      >
        <span slot="action" slot-scope="text, record">
          <a v-if="Number(record.taskStatus) === 0" @click="handleShowDetail(record)">检阅</a>
          <a v-if="Number(record.taskStatus) === 1" @click="handleShowDetail(record, 'detail')">查看详情</a>
        </span>
      </a-table>
...
import { setDoctor, setDoctorUpdate } from 'src/api/viewport/index'
import { getDoctorTaskList } from 'src/api/viewport/index'
import ViewportReviewModal from './modules/ViewportReviewModal.vue'

export default {
  name: 'ViewportReviewPackageDetail',
  ...
  methods: {
    ...
    handleShowDetail(record, type) {
      if (type === 'detail') {
        this.$router.push({
          path: `/viewportReviewDetail?id=${record.taskId}&orderId=${record.orderId}&resource=${record.dicomGroupId}&taskId=${record.taskId}&doctorId=${record.taskId}&backId=${this.$route.query.id}&backType=detail&state=admin`
        })
      } else {
        this.$router.push({
          path: `/viewportReviewDetail?id=${record.taskId}&orderId=${record.orderId}&resource=${record.dicomGroupId}&taskId=${record.taskId}&doctorId=${record.taskId}&backId=${this.$route.query.id}&backType=detail`
        })
      }
    },
    loadData() {
