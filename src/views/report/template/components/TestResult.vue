...
<div class="test-result">
 ...
 <h4>1. 生化分子组学（蛋白组学/代谢组学）检测结果</h4>
  <div class="test-result-content">
    ...
    <p>{{result.content}}
      <a-checkbox @change="onChange" v-model="riskLevel.high"></a-checkbox>高风险（70-100分）
      <a-checkbox @change="onChange" v-model="riskLevel.mid"></a-checkbox>中风险（50-70分）
      <a-checkbox @change="onChange" v-model="riskLevel.low"></a-checkbox>低风险（0-50分）
      {{suggest}}
    </p>
    <div class="hot-pic">
  ...
  <p class="text-indent">人工智能阅片分析，该受检者双肺共检出{{dicomResult.total}}个结节（具体信息详见下图），其中1个结节恶性程度评估为高风险（<span class="text-blue">结节恶性风险为：{{dicomResult.scrynMaligant}} %</span>）。具体信息为：</p>
</div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      dicomResult: {
        ...
        scrynMaligant: '',
        pulmonaryNodules: [],
        interpretation: dicom.interpretation
      }
    }
  },
  computed: {
  ...
  methods: {
    init () {
      ...
      this.result.content = `生化分子是生物功能的终端产物，其改变可反映生物体某一特定生理时期内的活动情况，与疾病表型关系也最为密切。蛋白大分子物质和代谢小分子物质都属于生物体内的生化分子。通过对肺癌发生发展中某些显著性差异的内源性分子物质的含量检测，经综合评估，该样本结节良恶性评分为${this.data.ananpanReportValue}分，肺结节恶性程度为:`
